...
export default function LamportTimestampsSimulator() {
  const [processes, setProcesses] = useState<Process[]>([
    { id: 'P1', name: 'Processo 1', clock: 0, events: [] },
    { id: 'P2', name: 'Processo 2', clock: 0, events: [] },
    { id: 'P3', name: 'Processo 3', clock: 0, events: [] },
  ]);
      ...
      <div className="py-8 md:py-12 px-4 md:px-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8 md:mb-12">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-3 md:mb-4">
            Simulador de Timestamps de Lamport
          </h1>
          <p className="text-base md:text-lg text-zinc-400">
            Visualize como os timestamps lógicos são atualizados em um sistema distribuído
          </p>
        </div>
        ...
        <div className="bg-blue-500/20 border border-blue-500 rounded-lg p-3 md:p-4 mb-6 md:mb-8">
          <div className="flex gap-3">
            ...
            <p className="text-sm md:text-base text-blue-300">
              Adicione eventos locais ou envie mensagens entre processos para ver como os timestamps de Lamport são atualizados.
              Observe como a ordem dos eventos é mantida através dos relógios lógicos.
            </p>
          </div>
        </div>
            ...
            <div key={process.id} className="flex flex-col">
              ...
              <div className="bg-zinc-800/50 rounded-lg p-3 md:p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-blue-400 font-mono text-sm md:text-base">t = {process.clock}</span>
                </div>
              ...
              <div className="flex-1 relative">
                ...
                <div className="space-y-3 md:space-y-4">
                  {process.events.map((event) => (
                    <div
                      key={event.id}
                      className="relative pl-6 md:pl-8 pr-4 md:pr-8"
                    >
                      {/* Timeline Node */}
                      ...
                      >
                        <div className="flex justify-between items-center text-xs md:text-sm">
                          ...
                          <span className="font-mono text-zinc-400">t = {event.timestamp}</span>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {/* Legend - Added for better understanding */}
        <div className="mt-8 bg-zinc-800/50 rounded-lg p-4">
          <h4 className="text-sm md:text-base font-medium text-white mb-3">Legenda</h4>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-green-500"></div>
              <span className="text-xs md:text-sm text-zinc-400">Evento Local</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-blue-500"></div>
              <span className="text-xs md:text-sm text-zinc-400">Mensagem Enviada</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-purple-500"></div>
              <span className="text-xs md:text-sm text-zinc-400">Mensagem Recebida</span>
            </div>
          </div>
        </div>
      </div>
