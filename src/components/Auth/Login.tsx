        ...
        <div className="bg-white dark:bg-slate-900/50 rounded-xl p-8 backdrop-blur-sm">
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="bg-red-500/10 border border-red-500/50 text-red-500 p-3 rounded-lg mb-4 text-sm text-center"
              >
                {error}
              </motion.div>
            )}
          ...
          <div className="space-y-4">
            <button
              ...
              disabled={isLoading}
              className="w-full bg-white text-zinc-900 py-3 px-4 rounded-lg font-medium hover:bg-zinc-100 transition-colors flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
