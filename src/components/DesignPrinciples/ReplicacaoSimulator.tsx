...
export default function ReplicacaoSimulator() {
  ...
  const processOperation = async (operation: Operation) => {
    ...
    if (operation.type === 'write') {
      ...
      // Handle replication based on type
  ...
  const toggleRegionStatus = (regionId: string) => {
    setRegions(prev => prev.map(region => {
      if (region.id === regionId) {
        return {
          ...region,
          status: region.status === 'healthy' ? 'failed' : 'healthy'
        };
      }
      return region;
    }));
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-4">Simulador de Replicação</h1>
          <p className="text-zinc-400 mb-6">
            Explore como diferentes estratégias de replicação afetam a consistência e disponibilidade dos dados.
          </p>
          ...
          <div className="bg-zinc-900/50 rounded-xl p-6 space-y-3 text-sm text-zinc-300">
            ...
            <ol className="list-decimal list-inside space-y-2">
              <li>Escolha o tipo de replicação:
                <ul className="list-disc list-inside ml-4 text-zinc-400">
                  <li><span className="text-blue-400">Síncrona</span>: Aguarda confirmação de todas as réplicas</li>
                  <li><span className="text-blue-400">Semi-síncrona</span>: Aguarda confirmação de pelo menos uma réplica</li>
                  <li><span className="text-blue-400">Assíncrona</span>: Não aguarda confirmação das réplicas</li>
                </ul>
              </li>
              <li>Ajuste os parâmetros:
                <ul className="list-disc list-inside ml-4 text-zinc-400">
                  <li>Latência de rede entre as regiões</li>
                  <li>Taxa de falha das operações</li>
                  <li>Número de réplicas ativas</li>
                </ul>
              </li>
              <li>Observe os resultados:
                <ul className="list-disc list-inside ml-4 text-zinc-400">
                  <li>Tempo de propagação dos dados</li>
                  <li>Estado da replicação em cada região</li>
                  <li>Impacto de falhas na consistência</li>
                </ul>
              </li>
            </ol>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              >
                ...
                <option value="semi-sync">Semi-síncrona</option>
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Número de Réplicas</label>
              <input
                type="range"
                min="1"
                max="3"
                value={config.replicaCount}
                onChange={(e) => setConfig(prev => ({ ...prev, replicaCount: Number(e.target.value) }))}
                className="w-full"
              />
              <span className="text-sm text-zinc-400">{config.replicaCount} réplicas</span>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${((region.latency + config.networkLatency) / 2000) * 100}%` }}
                    />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span>Dados</span>
                    <span>{Object.keys(region.data).length} chaves</span>
                  </div>
                ...
                >
                  {region.status === 'healthy' ? 'Simular Falha' : 'Recuperar'}
                </button>
              </div>
            </motion.div>
          ))}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-sm">
                        {operation.type === 'write' ? 'Escrita' : 'Leitura'} - 
                        Chave: {operation.key}
                        {operation.value && ` Valor: ${operation.value}`}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      {Object.entries(operation.replicationStatus).map(([regionId, status]) => (
                        <span
                          ...
                          title={`${regions.find(r => r.id === regionId)?.name}: ${status}`}
                        />
                      ))}
                    </div>
                  </div>
