        ...
        return (
          <div
            ...
            style={{
              left: `${left}%`,
              width: `${width}%`,
              top: `${spans.findIndex(s => s.id === span.id) * 32}px`,
            }}
          >
...
export default function TracingSimulator() {
  ...
  return (
    <div className="p-6 md:p-8 lg:p-12 max-w-7xl mx-auto">
      <div className="prose prose-invert prose-lg max-w-none mb-8">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl font-bold mb-4 text-blue-400">
            Simulador de Distributed Tracing
          </h1>
          <div className="flex gap-4">
            <button
              onClick={clearSimulation}
              className="px-6 py-3 bg-zinc-600 hover:bg-zinc-700 text-white rounded-lg transition-colors flex items-center gap-2"
            >
        ...
        <p className="text-xl text-zinc-300">
          Visualize como as requisições fluem através de diferentes serviços em um sistema distribuído.
          Cada requisição gera spans que mostram o tempo gasto em cada operação.
        </p>
      </div>

      {/* Controles de Simulação */}
      <div className="bg-zinc-900 p-6 rounded-lg mb-8">
        <h2 className="text-2xl font-bold text-zinc-200 mb-4">Controles</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-lg transition-colors flex items-center justify-center gap-2 ${
              !currentRequest || currentRequest.currentStep >= 5
                ? 'bg-zinc-700 text-zinc-400 cursor-not-allowed'
      ...
      {currentRequest && (
        <div className="bg-zinc-900 p-6 rounded-lg mb-8">
          <h2 className="text-2xl font-bold text-zinc-200 mb-4">Requisição Atual</h2>
          <div className="bg-black p-4 rounded-lg">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-blue-400">Trace ID: {currentRequest.id}</h3>
                <p className="text-sm text-zinc-400">
                  Início: {new Date(currentRequest.startTime).toLocaleTimeString()}
                </p>
              </div>
              <div className="text-sm text-zinc-400">
                Passo {currentRequest.currentStep} de 5
              </div>
            </div>
            ...
            <div className="space-y-4">
              {currentRequest.spans.map((span, index) => (
                <SpanView
                  key={span.id}
                  span={span}
                  hasChildren={currentRequest.spans.some(s => s.parentId === span.id)}
                  isLast={index === currentRequest.spans.length - 1}
                />
              ))}
            </div>
          </div>
      ...
      <div className="bg-zinc-900 p-6 rounded-lg">
        <h2 className="text-2xl font-bold text-zinc-200 mb-4">Histórico de Traces</h2>
            ...
            <div key={request.id} className="bg-black p-4 rounded-lg">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">
                    Início: {new Date(request.startTime).toLocaleTimeString()}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {request.spans.map((span, index) => (
                  <SpanView
                    key={span.id}
                    span={span}
                    hasChildren={request.spans.some(s => s.parentId === span.id)}
                    isLast={index === request.spans.length - 1}
                  />
                ))}
              </div>
            </div>
