...
const UserBadge = ({ role, color, className = '' }: UserBadgeProps) => {
  ...
  if (color) {
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full dark:rounded-none text-[10px] font-mono uppercase tracking-wider font-medium border ${className}`}
        style={style}
      >
        {role}
      </span>
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full dark:rounded-none text-[10px] font-mono uppercase tracking-wider font-medium border ${getFallbackStyles(role)} ${className}`}
    >
      {role}
    </span>
  );
