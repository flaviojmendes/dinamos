...
interface AdminCard {
  to: string;
  /** i18n key under `admin.cards.<card>` providing `.title` and `.description`. */
  card: string;
      ...
      <div className="min-h-screen bg-canvas-paper dark:bg-canvas-dark flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-slate-600 dark:text-tactical-dim">
            {t('admin.access_denied', { defaultValue: 'Access Denied' })}
          </p>
        </div>
      </div>
    );
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-sans font-bold tracking-tight text-slate-900 dark:text-tactical-text">
            {t('admin.hub.title', { defaultValue: 'Admin Control Center' })}
          </h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-tactical-dim">
            {t('admin.hub.subtitle', { defaultValue: 'Everything you can manage, in one place.' })}
          </p>
        </div>
